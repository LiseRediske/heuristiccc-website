...
export default function AIAssistantWidget() {
  ...
  const writeLeadSummaryToForms = (summary) => {
    ...
    if (strategyField) {
      strategyField.value = safeSummary;
      strategyField.setAttribute("value", safeSummary);
    }

    const frameworkField = document.getElementById("framework_lead_summary");
    if (frameworkField) {
      frameworkField.value = safeSummary;
      frameworkField.setAttribute("value", safeSummary);
    }
  };
  ...
  const handleSend = (rawMessage) => {
    ...
    const updatedState = applyPendingQuestion(workingState, message);
    ...
    if (
      ...
      const summary = `Lead Intent: Strategy Call | Business Type: ${updatedState.businessType} | Focus Area: ${updatedState.bottleneck}`;

      reply = {
        intent: "strategy_call",
        text: `It sounds like you're exploring AI automation for a ${updatedState.businessType} and focusing on improving ${updatedState.bottleneck}. A short strategy call is usually the fastest way to determine whether automation or system redesign makes sense. You can book directly, go to the strategy call form, or download the framework.`,
        summary,
        ctas: [
          {
          ...
          {
            label: "Go to Strategy Call Form",
            href: "#contact",
          },
          {
        ...
        nextState: {
          intent: null,
          pendingQuestion: null,
          businessType: updatedState.businessType,
          bottleneck: updatedState.bottleneck,
          leadSummary: summary,
        },
      };
    } else {
              ...
              >
                ...
                >
                  ...
                  {msg.role === "assistant" &&
                    ...
                    !msg.summary &&
                    !(
                      state.pendingQuestion === "businessType" ||
                      state.pendingQuestion === "bottleneck"
                    ) && (
                  ...
                  {msg.ctas?.length > 0 && (
                    <div className="mt-3 flex flex-col gap-2">
                      {msg.ctas.map((cta) => {
                        ...
                        const isInternalAnchor = cta.href.startsWith("#");
                        ...
                        if (isInternalAnchor) {
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                writeLeadSummaryToForms(
                                  msg.summary || state.leadSummary
                                );
